...
const pctToAmount = (totalCents, pct) =>
  ...
  async (request) => {
    try {
      ...
      if (
        ...
        commissionPct === null ||
        isNaN(Number(commissionPct)) ||
        Number(commissionPct) < 0 ||
        Number(commissionPct) > 100
      ) {
        throw new HttpsError('invalid-argument', 'commissionPct must be between 0 and 100');
      }
      if (!amountUsd || isNaN(Number(amountUsd)) || Number(amountUsd) <= 0) {
        throw new HttpsError('invalid-argument', 'amountUsd must be a positive number');
      }
      if (!invoiceId || !barnId) {
      ...
      if (inv.status !== 'pending') {
        throw new HttpsError('failed-precondition', 'Only pending invoices can be paid');
      }

      const totalCents = toCents(amountUsd);
      const feeCents = pctToAmount(totalCents, commissionPct);
      ...
      const metadata = {
        ...
        applicationFeePct: `${commissionPct}%`,
        invoice_id: invoiceId,
        barn_id: barnId,
        horse_name: inv.horse_name || '',
        boarder_name: inv.boarder_name || '',
        application_fee_pct: `${commissionPct}%`,
      };

      const idempotencyKey = `direct_pi_${uid}_${invoiceId}_${totalCents}_${currency}_${connectedAccountId}`;
